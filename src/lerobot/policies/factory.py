...
def make_pre_post_processors(
    ...
    **kwargs: Unpack[ProcessorConfigKwargs],
    ...
    """
    if isinstance(policy_cfg, RLTConfig):
        # RLT acts in the frozen PI05 checkpoint's normalized action space.
        # New rollout statistics must never silently change that coordinate system.
        base_path = policy_cfg.base_policy_path
        base_cfg = PreTrainedConfig.from_pretrained(base_path)
        if base_cfg.type != "pi05":
            raise ValueError("RLT processors require a frozen PI05 base checkpoint.")
        base_cfg.device = policy_cfg.device
        return make_pre_post_processors(base_cfg, pretrained_path=base_path, **kwargs)
