...
class PolicyServer(services_pb2_grpc.AsyncInferenceServicer):
    ...
    def _validate_training_time_rtc_setup(self, policy_specs: RemotePolicyConfig) -> None:
        ...
        if policy_specs.policy_type == "rlt" and not training_time_rtc:
            raise ValueError("Remote RLT deployment requires `training_time_rtc=true`.")
        if not training_time_rtc:
            return

        if policy_specs.policy_type not in {"pi05", "rlt"}:
            raise ValueError("Training-Time RTC inference supports `policy_type=pi05` or `policy_type=rlt`.")
        ...
        if policy_specs.policy_type == "rlt":
            actor_prefix = getattr(self.policy.config, "rtc_prefix_steps", None)
            if actor_prefix is None or configured_prefix != actor_prefix:
                raise ValueError(
                    f"RLT requires the prefix used for actor training: d={actor_prefix}, "
                    f"requested d={configured_prefix}. Rebuild the cache and retrain to change it."
                )
            if policy_specs.actions_per_chunk != model_chunk_size:
                raise ValueError("RLT deployment must request the full chunk_size used for actor training.")
