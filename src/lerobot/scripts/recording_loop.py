    ...
    while timestamp < control_time_s:
        ...
        if events.get("toggle_intervention", False):
            ...
            if intervention_enabled:
                if intervention_state == INTERVENTION_STATE_POLICY:
                    ...
                    if remote_policy_client is not None and hasattr(remote_policy_client, "suspend"):
        ...
        if act_processed_policy is None and act_processed_teleop is None:
            if not no_action_warned:
                logging.info(
                    "No policy or teleoperator provided; waiting for the reset interval without sending actions. "
                    "The operator must reset the robot and environment before the next episode."
                )
                no_action_warned = True
            # Pure policy rollouts have no teleoperator during the reset phase.
            # Advance wall time here too, otherwise continue bypasses the loop's
            # normal clock update and the reset interval never finishes.
        ...
        if selected_from_policy and remote_policy_client is not None and hasattr(
            ...
            remote_policy_client.confirm_action_executed(_sent_action)

        # The environment transition uses the accepted hardware command, including
        # any robot-side clipping, rather than the requested target.
        if not isinstance(_sent_action, dict):
            raise TypeError("robot.send_action must return the action actually sent as a dictionary.")
