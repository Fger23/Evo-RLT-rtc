...
class TrainedRTCInferenceConfig:
    ...
    # RLT exports preserve the PI0.5 learned-RTC transport contract.
    policy_type: str = "pi05"

    def __post_init__(self) -> None:
        if self.policy_type not in {"pi05", "rlt"}:
            raise ValueError("`policy_type` must be pi05 or rlt for trained RTC inference.")
...
class TrainedRTCInferenceClient:
    ...
    def _start_request(self, observation: dict[str, Any]) -> bool:
        ...
        with self._state_lock:
            ...
            if getattr(self.cfg, "policy_type", "pi05") == "rlt" and 0 < prefix_steps < self.cfg.rtc_prefix_steps:
                # The residual actor was trained with its configured prefix (or
                # a cold start). Finish this small safe queue before bootstrapping
                # with d=0 rather than requesting an unseen intermediate delay.
                return False
