...
class RemotePolicyRecordConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if self.training_time_rtc:
            if self.policy_type not in {"pi05", "rlt"}:
                raise ValueError("Trained RTC recording supports policy_type=pi05 or rlt.")
            if self.aggregate_fn_name != "latest_only":
                raise ValueError("Trained RTC recording requires aggregate_fn_name=latest_only.")
            if self.rtc_d_monitor_address:
                raise ValueError("Trained RTC reports confirmed-step delay in its own client logs.")
